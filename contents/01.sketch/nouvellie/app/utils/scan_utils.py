...
class Scanner:
    ...
    def osx(cls, lat, lon, comment):
        ...
        t = subprocess.Popen(
            args=[cfg.FP_AIRPORT, '-s', '-x'], stdout=subprocess.PIPE)
        ...
        for network in scan_xml:
            ...
            bssid = ':'.join(map(lambda x: x if (len(x) == 2)
                                 else ('0'+x), network['BSSID'].split(':')))
            ssid = network['SSID_STR']
            channel = network['CHANNEL']
            rssi = network['RSSI']
            if not bssid in networks.keys():
                networks[bssid] = WiFiNet(
                    ssid, bssid, channel, rssi, vendor=self.mac2vendor(bssid))
